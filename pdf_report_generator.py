...
# Paths
# ---------------------------
ROOT = Path(__file__).resolve().parent.parent   # .../KPMG
DATA = ROOT / "data"
OUT  = ROOT / "outputs"
ASSETS = ROOT / "pdfoutput" / "assets"          # optional for logo
...
df["sla_target"] = np.where(df["sla_days"]>0,
...
due_soon_count = int(((df["sla_target"] - now).dt.days <= 7).sum())
...
theme_grp = (df.groupby("theme", dropna=False)
...
def save_compliance_bar(theme_grp):
    ...
    bars = ax.bar(comp_order, counts.values, color=[comp_colors[c] for c in comp_order], edgecolor="black")
    for i, v in enumerate(counts.values):
        ax.text(i, v + 0.3, str(int(v)), ha="center", va="bottom", fontsize=9)
...
# Slightly wider 3rd column so % doesn't crowd
...
kpi_tbl.setStyle(TableStyle([
    ...
    ("FONTSIZE",     (0,0), (-1,0),  10),     # header size
    ("ALIGN",        (0,0), (-1,-1), "CENTER"),
    ("VALIGN",       (0,0), (-1,-1), "MIDDLE"),
    ("FONTSIZE",     (0,1), (-1,1),  18),     # big numerals
...
# Apply severity colors to the severity column
...
rs = TableStyle([("GRID",(0,0),(-1,-1),0.25,colors.grey),
                 ("BACKGROUND",(0,0),(-1,0),colors.lightgrey),
                 ("FONTNAME",(0,0),(-1,0),"Helvetica-Bold"),
                 ("VALIGN",(0,0),(-1,-1),"TOP")])
