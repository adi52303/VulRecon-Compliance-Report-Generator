...
- Uses paths relative to this script (works with OneDrive/Desktop/etc.)
...
# Paths & IO helpers
# ----------------------------
ROOT = Path(__file__).resolve().parent           # .../KPMG
DATA_DIR = ROOT / "data"
OUT_DIR = ROOT / "outputs"
OUT_DIR.mkdir(parents=True, exist_ok=True)

FINDINGS_CSV = DATA_DIR / "sample_findings.csv"
MAPPING_CSV  = DATA_DIR / "iso27001_mapping_scaffold.csv"  # or .xlsx if you use excel


def fail_if_missing(path: Path, label: str):
    if not path.exists():
        print(f"❌ Missing {label}: {path}")
        # Show a hint of where we looked from
        print(f"   (Working dir: {Path.cwd()})")
        sys.exit(1)


fail_if_missing(FINDINGS_CSV, "findings CSV")
fail_if_missing(MAPPING_CSV, "mapping CSV")
...
def load_mapping(path: Path) -> pd.DataFrame:
    ...
    for c in wanted:
        if c not in df.columns:
            # Create blank column if absent (lets script run even with partial mapping files)
            df[c] = None
...
# Build a row-wise mapping result (returns the 4 columns)
